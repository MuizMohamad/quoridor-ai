...
stepToEdgeTwo :: Step -> [(Index, Index)]
stepToEdgeTwo (c, c') = stepToEdge (c,middleCell) ++ stepToEdge (middleCell,c') 
        where middleCell = head [ x | x <- cellsAroundInBoard c , x `elem` (cellsAroundInBoard c') ]

    
stepToEdge :: Step -> [(Index, Index)]
stepToEdge (c, c') 
    | isTwoAdjacent c c' = stepToEdgeTwo (c, c')
    | otherwise = [(cellToIndex c, cellToIndex c')]
